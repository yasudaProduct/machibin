...
| 1.4 | 2026-07-12 | Claude | 認証手段にAppleソーシャルログインを追加（BD-02改訂）。iOSでのSign in with Apple併設義務（App Store審査ガイドライン4.8）への対応をPhase 1スコープへ組み込んだ。 |
...
1. **実装・運用コストの削減** — サインアップ・ログイン・セッション管理・トークンローテーション・不正ログイン検知等を自前実装せず、Expo公式SDK（`@clerk/expo`）とバックエンド検証で完結する。認証手段はGoogle・Appleの2つのソーシャルログインのみとし（BD-02）、OTP・パスワード管理を自前で作り込む工数が丸ごと不要になる。iOSでサードパーティのソーシャルログイン（Google）を提供する以上、App Store審査ガイドライン4.8によりSign in with Appleの併設が必須となるため、両プロバイダともClerkダッシュボードでの有効化のみで追加できる点も採用の後押しとなる。
...
| Expo Push | 端末への通知配信 |
| Resend | 将来のトランザクションメール送信用に予約（Phase 1は認証をGoogle・Appleのソーシャルログインのみとするため出番なし） |
...
1. 初回起動：Clerkのサインアップ／ログインUI（`@clerk/expo`）で**Google・Appleのソーシャルログインのみ**を行い、Clerkがセッショントークン（JWT）を発行する（メールOTP・パスワード・マジックリンク等は提供しない）。
...
| メール（将来用） | Resend 無料枠 | **$0** | 3,000通/月。Phase 1は認証をGoogle・Appleのソーシャルログインのみとするため未使用。超過時はSESや上位プランへ。 |
...
- Clerkのオンボーディングフロー（Google/Apple OAuthのスコープ・同意画面の文言等）の確定と、Neon側プロフィールとのWebhook連携設計。
