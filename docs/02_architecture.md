...
| 非同期・スケジューリング | **Cloudflare Cron Triggers**（＋必要に応じ Queues / Durable Objects Alarms） | 配達遅延演出をバッチで実現。無料枠内、常駐不要 | 外部cron、SQS+Lambda |
...
1. **実装・運用コストの削減** — サインアップ・ログイン・セッション管理・トークンローテーション・不正ログイン検知等を自前実装せず、Expo公式SDK（`@clerk/expo`）とバックエンド検証で完結する。OTP設計を自前で作り込む工数が丸ごと不要になる。
...
| Cron Triggers（配達バッチ） | 配達時刻に達した交換の確定と、受信者へのプッシュ通知送信 |
...
本サービスの中核サイクル。**配達はリアルタイムに行わず、配達ウィンドウ（例：朝・夕）にバッチ実行する。** これは「翌朝ポストに届く」体験（要件 FR-05）に合致すると同時に、後述のコスト最適化（DBのスケールtoゼロを維持）にも寄与する。
...
sequenceDiagram
    ...
    API->>DB: スポット登録 + 抽選クレジット付与
    API->>DB: プールからランダム抽選<br/>（自分/受信済/ブロックを除外）
    API->>DB: 交換レコード作成<br/>(deliver_at=配達時刻, status=scheduled)
    API-->>App: 受理（配達待ち表示）
    Note over Cron,DB: 配達ウィンドウで実行（例：毎朝/毎夕）
    Cron->>DB: deliver_at<=now() かつ scheduled を取得
    Cron->>DB: status=delivered に更新
    Cron->>Push: 受信者へ通知送信
    Push-->>R: 「新しい場所が届きました」
...
1. 初回起動：Clerkのサインアップ／ログインUI（`@clerk/expo`の`<AuthView/>`またはカスタムフロー）でメール認証（コード／マジックリンク等）を行い、Clerkがセッショントークン（JWT）を発行する。
...
| データベース | Neon 無料枠 | **$0** | 0.5GB・100 CU-hours/月・スケールtoゼロ。配達をバッチ化しアイドルを維持（8.4参照）。 |
...
| メール(OTP) | Resend 無料枠 | **$0** | 3,000通/月。超過時はSESや上位プランへ。 |
...
- **配達バッチ化でNeonのアイドルを維持:** 分単位でDBをポーリングするCronは、Neonのスケールtoゼロを妨げて無料枠（100 CU-hours）を早期に消費する。配達は1日数回のウィンドウ（朝・夕など）に限定し、DBを普段は休止させる。これは「翌朝届く」UXとも一致する。
...
- 配達ウィンドウの回数・時刻の確定（UXとコストのバランス）。
- 抽選クエリの規模対応方式（`TABLESAMPLE`／乱数キー）の実装方針決定。
- POIスナップ導入時のデータソース選定（OSM/ジオコーダの無料枠・自ホスト可否・権利確認）。
- Clerkのオンボーディングフロー（メールコード／マジックリンク／パスキー等）の確定と、Neon側プロフィールとのWebhook連携設計。
