...
| 課題 | 関連文書 | 暫定方針 |
| --- | --- | --- |
| `idempotencyKey` の永続化設計がない（保存先テーブル・保持期間・レスポンス再現方法が未定義） | 06、07 API-20 | 専用テーブル（key＋初回レスポンス＋TTL）を06に追加する方向で検討し、実装PRで06を改訂 |
| 通知許可の要求タイミングが 05 §4.4（初回設定完了直後）と 09 §6.1（初回投函完了直後・プレプロンプト付き）で矛盾 | 05、09 | 09案（許可率向上の根拠が明記されている）へ統一し、05を改訂 |
