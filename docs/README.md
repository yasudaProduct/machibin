# まちびん 設計ドキュメント

**プロダクト名:** まちびん
**サブタイトル:** 「おすすめ場所」ランダム交換サービス

本ディレクトリは、まちびんの要件定義・設計ドキュメント一式を管理する。本書はその目次であり、各文書の位置づけ・記載ルールを定める。

---

## 1. ドキュメント一覧

| No | 文書 | 内容 | 工程 |
| --- | --- | --- | --- |
| 01 | [要件定義書](./01_requirements.md) | 提供価値、機能要件（FR-01〜14）、非機能要件（NFR）、データ要件、外部IF要件 | 要件定義 |
| 02 | [アーキテクチャ設計書](./02_architecture.md) | 技術スタック選定、システム構成、主要データフロー、コスト試算、スケール戦略 | 方式設計 |
| 03 | [システム構成設計書](./03_system_configuration.md) | システム関連図、アーキテクチャ構成図、インフラ構成図、環境定義 | 基本設計 |
| 04 | [機能設計書](./04_functional_design.md) | FR-01〜10の処理仕様（入力/処理/出力/例外）、業務ルール、設定パラメータ | 基本設計 |
| 05 | [画面設計書](./05_screen_design.md) | 画面遷移図、画面レイアウト、表示・入力項目、入力チェック | 基本設計 |
| 06 | [データベース設計書](./06_database_design.md) | ER図、テーブル定義、コード定義、インデックス方針 | 基本設計 |
| 07 | [API設計書](./07_api_design.md) | REST APIエンドポイント仕様、認証、エラーコード、レート制限 | 基本設計 |
| 08 | [バッチ・非同期処理設計書](./08_batch_design.md) | 配達バッチ等のジョブ定義、スケジュール、冪等性、リトライ | 基本設計 |
| 09 | [通知設計書](./09_notification_design.md) | プッシュ通知の種別・文面・ペイロード、通知設定、トークン管理 | 基本設計 |
| 10 | [認証・認可設計書](./10_auth_design.md) | Clerk認証フロー、JWT検証、ロール・権限マトリクス、退会処理 | 基本設計 |
| 11 | [プライバシー・データ保護設計書](./11_privacy_design.md) | 位置情報丸め、データ分類・保持期間、削除フロー、法令対応 | 基本設計 |
| 12 | [開発標準・リポジトリ構成ガイド](./12_development_standards.md) | モノレポ構成、型共有方針、コーディング規約、Git運用、スパイク一覧（SP-xx）、既知の設計課題 | 実装準備 |
| 13 | [テスト設計書](./13_test_design.md) | テスト戦略・環境、重点テストケース（TC-xx）、リリース前チェックリスト | 実装準備 |
| 14 | [環境構築手順書](./14_setup_guide.md) | 外部サービス（Cloudflare / Neon / Clerk / Apple / Google / EAS / Sentry）のセットアップRunbook | 実装準備 |
| 15 | [地図タイル配信設計書](./15_map_tiles_design.md) | PMTiles生成パイプライン、配信Worker、MapLibreスタイル・更新運用 | 基本設計 |

### 付属文書（公開文書ドラフト）

設計書ではないが、本リポジトリで管理する公開予定文書。**公開前に専門家レビュー必須**（11 §9.5）。

| 文書 | 内容 |
| --- | --- |
| [利用規約（ドラフト）](./legal/terms_of_service.md) | BD-03/06/08、FR-08等を反映した利用条件のドラフト |
| [プライバシーポリシー（ドラフト）](./legal/privacy_policy.md) | 11 §9.2の記載必須項目に基づくドラフト |
| [Phase 1 実装計画](./plans/phase1_implementation_plan.md) | マイルストーン（M0〜M7）、ユーザー作業レーン、文書改訂割当、リスク。実装進行にあわせて更新する運用文書 |

### 文書間の関係

```mermaid
flowchart TB
    R01["01 要件定義書<br/>（WHAT / WHY）"] --> R02["02 アーキテクチャ設計書<br/>（技術方式のHOW）"]
    R02 --> D03["03 システム構成設計書"]
    R01 --> D04["04 機能設計書"]
    D04 --> D05["05 画面設計書"]
    D04 --> D06["06 データベース設計書"]
    D04 --> D07["07 API設計書"]
    D06 --> D07
    D04 --> D08["08 バッチ・非同期処理設計書"]
    D04 --> D09["09 通知設計書"]
    R02 --> D10["10 認証・認可設計書"]
    R01 --> D11["11 プライバシー・データ保護設計書"]
    R02 --> D12["12 開発標準・リポジトリ構成ガイド"]
    D04 --> D13["13 テスト設計書"]
    D07 --> D13
    D10 --> D13
    D03 --> D14["14 環境構築手順書"]
    R02 --> D15["15 地図タイル配信設計書"]
    D11 --> L01["利用規約・プライバシーポリシー（ドラフト）"]
```

---

## 2. 基本設計の前提（暫定決定事項）

要件定義書・アーキテクチャ設計書で「基本設計で決定する」とされていた事項のうち、以下は基本設計の前提として**暫定決定**した。変更する場合は関連文書を改訂する。

| No | 決定事項 | 内容 | 主な関連文書 |
| --- | --- | --- | --- |
| BD-01 | 配達タイミング | **即時配達**。投函（API-20）時に抽選候補が見つかれば同一トランザクション内で配達まで完了させ、画面上は数秒の演出後にSCR-05へ自動遷移する。在庫不足時のみ`pending`とし、数分間隔の非同期ジョブ（JOB-02）が見つかり次第配達する。朝夕2回等の定期配達バッチ（旧案）は廃止。距離連動の配達演出・定期便モードは将来のPhase 2以降オプションとする | 04, 05, 06, 07, 08, 09 |
| BD-02 | 認証手段 | **Google・Appleの2つのソーシャルログインのみ**（Clerk）。メールOTP・パスワードは提供しない。iOSでサードパーティのソーシャルログイン（Google）を提供する以上、App Store審査ガイドライン4.8によりSign in with Appleの併設が必須となるため、Phase 1から両方を実装する | 05, 10 |
| BD-03 | 年齢方針 | **13歳以上・自己申告**。登録時に13歳以上であることの同意チェックのみ行い、生年月日は収集しない（データ最小化） | 04, 10, 11 |
| BD-04 | 運営機能の形態 | **管理者ロール付きの管理APIのみ**（Phase 1では管理画面を作らない）。シード投入・通報対応はAPIで運用 | 04, 07, 10 |
| BD-08 | 未成年者対応 | 13歳以上・自己申告方針（BD-03）を維持しつつ、利用規約に「未成年者は保護者の同意を得た上で利用すること」の一文を追加する（民法上の契約取消しリスクへの実務的な緩和策） | 10, 11 |
| BD-05 | 対応OS下限 | iOS 15.1以上 / Android 7.0（API 24）以上（Expo SDKのサポート範囲に準拠。実装時のSDKバージョンで再確認） | 03, 05 |
| BD-06 | 退会時データの扱い | 本人データ・投稿スポットは削除、配達済みスポットは**受信者の場所帳にスナップショットとして匿名化残置** | 06, 10, 11 |
| BD-07 | アバター | Phase 1は**プリセット選択式**（画像アップロードなし。モデレーション不要化） | 04, 05 |

---

## 3. 記載ルール

### 3.1 文書の共通構成

各設計書は次の構成に従う。

1. 冒頭に「改訂履歴」「文書情報」（文書名／ステータス／関連文書）を置く。
2. 図は Mermaid、定義は表で記載する。
3. Phase 1（MVP）の仕様を主として記述し、Phase 2以降は「拡張方針」として区別する。
4. 運用で調整可能な値は、機能設計書の設定パラメータ一覧（PRM-xx）を正とし、他文書からはIDで参照する。

### 3.2 ID体系

| プレフィックス | 意味 | 定義元 |
| --- | --- | --- |
| FR-xx | 機能要件 | 01 要件定義書 |
| NFR-xx | 非機能要件 | 01 要件定義書 |
| UC-xx | ユースケース | 01 要件定義書 |
| SCR-xx | 画面 | 05 画面設計書 |
| TBL-xx | テーブル | 06 データベース設計書 |
| CD-xx | コード（区分値） | 06 データベース設計書 |
| API-xx | APIエンドポイント | 07 API設計書 |
| JOB-xx | バッチジョブ | 08 バッチ・非同期処理設計書 |
| NT-xx | 通知 | 09 通知設計書 |
| PRM-xx | 設定パラメータ | 04 機能設計書 |
| BD-xx | 基本設計の暫定決定事項 | 本書 |
| TC-xx | テストケース | 13 テスト設計書 |
| SP-xx | 実装前スパイク（技術検証） | 12 開発標準・リポジトリ構成ガイド |

---

*本目次は文書の追加・改訂にあわせて更新する。*
