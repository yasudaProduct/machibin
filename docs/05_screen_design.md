...
| 1.3 | 2026-07-21 | Claude | 通知許可の要求タイミングを09通知設計書§6.1に統一（初回設定完了直後→初回投函完了直後・SCR-04初回表示時のプレプロンプト方式へ変更）。SCR-04にプレプロンプトのUI仕様を追加（12 §11.2の既知課題を解消） |
| 1.4 | 2026-07-25 | Claude | suspended状態のアクセス範囲に関する誤り（§4.5「閲覧系は継続可」が07 §4・10 §7.2・04 G-04と矛盾）を修正。SCR-12（アカウント停止画面）を追加し、ガード遷移（§3.3）にsuspended判定（G-02）を新設 |
...
| SCR-12 | アカウント停止画面 | 停止中である旨の表示、ログアウト・退会のみ提供 | API-10, 12, 15 | **追加画面**。suspendedユーザーはAPI-10/12/15以外が一律403となる（04 G-04、07 §4、10 §7.2）ため、他画面に遷移させず専用の状態画面へ誘導する必要があるため |
...
| 停止スタック | `status = suspended` の間のみ表示。タブなし・他領域への遷移不可 | SCR-12 |
...
- 停止スタック（SCR-12）はボトムタブを持たず、ログアウト・退会以外の遷移導線を一切提供しない（§3.3 G-02）。
...
flowchart TD
    ...
    SCR01 -- "ソーシャルログイン成功" --> StatusCheck{"status<br/>（API-10）"}
    AuthCheck -- "認証済み" --> StatusCheck
    StatusCheck -- "suspended" --> SCR12["SCR-12<br/>アカウント停止"]
    StatusCheck -- "active" --> OnbCheck{"onboardingCompleted<br/>（API-10）"}
...
| G-02 | 認証済みかつ `status = suspended`（API-10） | SCR-12 | オンボーディング未完了かどうかに関わらず優先する（G-03より先に判定）。suspendedはAPI-10・API-12・API-15以外が一律403のため（04 G-04、07 §4、10 §7.2）、他画面へは一切進ませない。API 403 ACCOUNT_SUSPENDED受信時も同様（§4.5） |
| G-03 | 認証済み・active かつ `onboardingCompleted = false`（API-10） | SCR-10 | 完了までメイン領域へ進ませない。SCR-10は戻る操作で回避不可 |
| G-04 | 上記以外 | SCR-02（ホーム） | |
...
- 通知許可は初回投函完了直後（SCR-04 配達待ち画面の初回表示時）に要求する。初回起動直後・オンボーディング完了直後には要求しない（文脈のない要求は拒否されやすいため。許可率向上のためのUX判断。09 通知設計書 §6.1）。SCR-04上にプレプロンプト（要求理由の説明とボタン）を表示したのちOSダイアログを出す。許可時にAPI-14でトークンを登録する。ログアウト時はAPI-15で削除する。プレプロンプトの詳細仕様は§5.5（SCR-04）を参照。
...
| API 403（ACCOUNT_SUSPENDED） | ダイアログ「アカウントは現在停止されています」を表示後、SCR-12へ遷移する（replace）。以後利用できるのはAPI-10・API-12・API-15のみで、閲覧系を含むそれ以外は一律403となる（07 §4、10 §7.2、04 G-04）。セッション中に停止された場合（既にG-02判定を通過済みの画面での操作中に発生）を主に想定した経路であり、通常はアプリ起動・フォアグラウンド復帰・画面遷移時のG-02が先に働く |
...
| 操作 | 挙動 |
| --- | --- |
| 「はじめる」 | API-11 PATCH /me（nickname, avatarId, homeArea, ageConfirmed: true, tosAgreed: true）。成功時SCR-02へ |
...
本画面の**初回表示時**（在庫あり・在庫不足いずれの場合も）に、通知許可のプレプロンプトを表示する（09 通知設計書 §6.1）。初回投函の直後は「交換した場所がいつ届くか知りたい」という動機が最も強い瞬間であり、通知許可を求める理由が自明なタイミングであるため、初回起動時ではなく本画面を採用する。
...
| 通知許可プレプロンプト | 初回表示時のみ（在庫あり・在庫不足共通） | ボトムシート等で配達演出／待機表示に重ねて表示。「配達をお知らせするために通知を許可してください」等の要求理由＋「通知を許可する」「あとで」の2ボタン（09 §6.1） |

- プレプロンプトは、**この端末でユーザーが初めてSCR-04を表示したときのみ**表示する（ローカルに表示済みフラグを保持し、以降のSCR-04表示では出さない。「あとで」を選んだ場合の再要求はSCR-08から行う。09 §6.1）。
- 在庫あり時、プレプロンプト表示中はPRM-03の自動遷移タイマーを一時停止し、ユーザーがボタンを押下（またはOSダイアログを閉じる）した時点で再開する（許可の意思決定を演出の途中で打ち切らせないため）。
...
| 操作 | 挙動 |
| --- | --- |
| （在庫あり時）演出終了 | PRM-03「配達演出秒数」（既定8秒。プレプロンプト表示中の一時停止分を除く）経過後、ユーザー操作なしにSCR-05へ自動でreplace遷移する |
| 「ホームへ戻る」（在庫不足時のみ表示） | SCR-02へ |
| プレプロンプト「通知を許可する」（初回表示時のみ） | OS通知許可ダイアログを表示する。許可時はexpo-notificationsでトークンを取得しAPI-14で登録する（09 §5.2）。拒否時もエラー表示はせず通常のフローを継続する |
| プレプロンプト「あとで」（初回表示時のみ） | プレプロンプトを閉じる（OSダイアログは表示しない）。以降はSCR-08（通知設定）から再度要求できる |
...
- 通知許可プレプロンプトの表示要否（初回か否か）はローカルの表示済みフラグで判定する（OS側の許可状態とは独立に「一度だけ」表示する）。端末を機種変更した場合は新端末で再度表示され得る。
- Android 8.0以降の通知チャネル（09 §6.2）は、本画面のプレプロンプト表示より前（アプリ起動時等）にあらかじめ作成しておく（POST_NOTIFICATIONSランタイム許可の前提。09 §6.2）。
...
### 5.12 SCR-12 アカウント停止画面（追加画面）

#### 概要

運営処分により`status = suspended`となったユーザーが、認証後に必ず誘導される専用画面（§3.3 G-02）。**要件定義書 §4.4 の画面一覧にはない追加画面**である。suspendedはAPI-10・API-12・API-15以外のすべてのAPIが403 ACCOUNT_SUSPENDEDを返すため（04 G-04、07 §4、10 §7.2）、ボトムタブを含む通常のメイン領域には一切遷移させず、本画面からログアウトまたは退会のみを行わせる。停止解除（`active`への復帰）は運営のみが行える操作であり、アプリ側に解除申請・問い合わせ機能はPhase 1では設けない。
...
| 項目 | 取得元（API-10） | 仕様 |
| --- | --- | --- |
| 停止メッセージ | `status` | 固定文言「アカウントは現在停止されています」＋理由の一般的な説明文（通報理由等の個別詳細は表示しない） |
...
| 操作 | 挙動 |
| --- | --- |
| 「ログアウト」 | 確認ダイアログ→API-15（プッシュトークン削除）→Clerkサインアウト→SCR-01へ（SCR-08と同一の挙動。§5.9） |
| 「退会する」 | **二段確認**。(1) 影響説明ダイアログ「投函した場所と受け取った場所帳はすべて削除されます。この操作は取り消せません」→ (2) 最終確認ダイアログ（「退会する」を明示的に押下）→API-12 DELETE /me→ローカルデータ・セッション破棄→SCR-01へ（SCR-08と同一の挙動。§5.9） |

戻る操作（スワイプ・OSバックボタン）は無効化し、本画面から抜けさせない。

#### 使用API

API-10 GET /me（表示判定・status確認）、API-12 DELETE /me（退会）、API-15 DELETE /me/push-tokens（ログアウト）。

#### 特記事項

- 本画面到達後にフォアグラウンド復帰・画面遷移が発生した場合も、G-02の再判定により本画面に留まり続ける（`status`が`active`に復帰するまで）。
- 停止解除後の遷移: 次回のG-02判定で`status = active`を確認した時点でG-03/G-04に従い通常のメイン領域へ進む。ユーザーへの復帰通知はPhase 1では行わない（アプリを開き直すことで気づく想定）。
- 通知タップ（NT-01・NT-03）で起動した場合も、G-02が優先されるため本画面が表示され、通知の本来の遷移先（SCR-05・SCR-11）へは進まない。
