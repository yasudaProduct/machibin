...
| 表示言語 | 日本語のみ（NFR-C02） |
| 画面向き | 縦固定 |
| 認証 | Clerk（`@clerk/expo`）。Googleソーシャルログインのみ（BD-02） |
...
| --- | --- | --- | --- | --- |
| SCR-01 | 起動・ログイン画面 | 「Googleでログイン」ボタン1つによるGoogle OAuth認証（Clerk。BD-02） | （Clerk SDK） | |
...
flowchart TD
    ...
    SCR01 -- "Googleログイン成功" --> OnbCheck{"onboardingCompleted<br/>（API-10）"}
...
Clerk（`@clerk/expo`）が提供するGoogle OAuthフローによるソーシャルログインのみを行う（BD-02）。画面に表示するのは「Googleでログイン」ボタン1つのみであり、メールアドレス入力・パスワード入力・確認コード（OTP）入力は一切行わない。ボタンをタップするとブラウザ／ネイティブのGoogle認証画面に遷移し、認証完了後にアプリへ戻ってセッションを確立する。新規登録とログインは区別しない（Googleアカウントの登録有無に応じてClerkが内部でSign up／Sign inフローを切り替える）。
...
| 「Googleでログイン」ボタン | 唯一の認証導線。Googleブランドガイドラインに準拠したボタン意匠とする |
| 補足テキスト | 「タップするとGoogleの認証画面に移動します」 |

#### 入力項目とバリデーション

テキスト入力フォームなし（Googleの認証画面側での入力に委譲するため、本画面自体に入力項目・バリデーションは存在しない）。
...
Clerk SDK（`@clerk/expo`）のGoogle OAuthフローのみ。認証成功後にAPI-10 GET /me を取得する。

#### 特記事項

- メールアドレス＋パスワード認証、Google以外のSNSログイン、メール確認コード（OTP）は提供しない（BD-02）。
- 利用規約・年齢確認の同意はSCR-10で行う（本画面では求めない）。
- Googleアカウントから取得できる氏名・プロフィール画像は、SCR-10のニックネーム・アバターに自動転記しない。ニックネームは引き続き入力必須、アバターはプリセット選択式（BD-07）のままとし、匿名性方針との整合を保つ（§5.2）。
